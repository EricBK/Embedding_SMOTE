import numpy as np
import Embedding.Auto_encoder.deepautoencoder.utils as utils
...
class StackedAutoEncoder:
    ...
    def __init__(self, dims, activations, epoch=1000, noise=None, loss='rmse',
                 lr=0.001, batch_size=100, print_step=50,use_latest_para=True):
        ...
        self.use_latest_para = use_latest_para
    ...
    def fit(self, x):
        if self.use_latest_para:
            if os.path.exists('./Embedding/Auto_encoder/deepautoencoder/parameters.pkl'):
                with open('./Embedding/Auto_encoder/deepautoencoder/parameters.pkl','rb') as file:
                ...
                print('No trained model, fit now...')
        ...
        # 将模型训练的权重文件保存下来
        parameters = {'weights':self.weights,'bias':self.biases,'activations':self.activations}
        with open('./Embedding/Auto_encoder/deepautoencoder/parameters.pkl','wb') as file:
