...
from Embedding.fm.utilities import *
import logging
...
class FM(object):
    ...
    def inference(self):
        ...
        with tf.variable_scope('interaction_layer'):
            v = tf.get_variable('v', shape=[self.p, self.k],dtype=tf.float32,
...
def check_restore_parameters(sess, saver):
    ...
    ckpt = tf.train.get_checkpoint_state("./Embedding/fm/checkpoints")
...
def train_model(sess, model, epochs=10, print_every=50):
    ...
    train_writer = tf.summary.FileWriter('./Embedding/fm/train_logs', sess.graph)
    # get sparse training data
    with open(args.train_onehot, 'rb') as f:
    ...
    for e in range(epochs):
        ...
        for ibatch in range(num_batches):
            ...
            if actual_batch_size != model.batch_size: continue
            ...
            if global_step % print_every == 0:
                ...
                saver.save(sess, "./Embedding/fm/checkpoints/model", global_step=global_step)
    ...
    plt.plot(loss_list)
    plt.show()
def my_test_model(sess,model, print_every = 50):
    """testing model"""
    # get testing data, iterable
    with open(args.test_onehot, 'rb') as f:
    ...
    for ibatch in range(num_batches):
        ...
        batch_pred = y_out_prob[0][:, -1]
        all_ids.extend(batch_ids)
        all_true_labels.extend(batch_y)
        all_prediction.extend(batch_pred)
    pd.DataFrame(np.array([all_ids, all_true_labels,all_prediction]).T, columns=['id', 'label','prediction']).to_csv('./Embedding/fm/result_regl1_.csv', index=False)
def get_feature_length():
    with open('./parameters.conf', 'r') as file:
        parameters = file.readline()
        if parameters:
            leaves_num = int(parameters.split(':')[1])
        else:
            leaves_num = 130
    return leaves_num
if __name__ == '__main__':
    ...
    train_array_length = get_feature_length()
    test_array_length = train_array_length
    # initialize the model
    config = {}
    config['lr'] = args.lr
    config['batch_size'] = args.batch_size
    config['reg_l1'] = args.reg_l1
    config['reg_l2'] = args.reg_l2
    config['k'] = args.k
    # get feature length
    ...
    with tf.Session() as sess:
        ...
        if mode == 'train':
            ...
            train_model(sess, model, epochs=args.epochs, print_every=500)
        if mode == 'test':
            print('start testing...')
            my_test_model(sess, model)